...
function createData(name, email, phone, prix, company, joinDate, status) {
  return { name, email, phone, prix, company, joinDate, status };
}

let STATUSES = ["Promotion", "Disponible", "Repture"];
const rows = [
  createData(
    faker.name.findName(),
    faker.internet.email(),
    faker.phone.phoneNumber(),
    faker.random.number({ min: 7 || 0, max: 30 }),
    faker.random.number({ min: 0 || 0, max: 100 }),
...
const headCells = [
  ...
  { id: "prix", numeric: false, disablePadding: false, label: "Prix" },
  {
    id: "Joining_Date",
    numeric: false,
    disablePadding: false,
    ...
    >
      ...
      {numSelected > 0 ? (
        <Tooltip title="Add Product">
          <IconButton aria-label="add" className={classes.iconHide}>
            <AddIcon />
          </IconButton>
        </Tooltip>
      ) : (
                        ...
                        <Typography
                          ...
                          style={{
                            backgroundColor:
                              ...
                              (row.status === "Repture" && "red") ||
                              (row.status === "Promotion" && "orange"),
                          }}
                        >
                          {row.status}
                        </Typography>
